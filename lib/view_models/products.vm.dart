...
class ProductViewModel extends MyBaseViewModel {
  ...
  void newProduct() async {
    final result = await viewContext.push((context) => NewProductPage());
    //
    if (result != null) {
      fetchMyProducts();
    }
  ...
  processStatusUpdate(Product product) async {
    ...
    try {
      final apiResponse = await productRequest.updateDetails(product);
      //
  ...
  processDeletion(Product product) async {
    ...
    setBusyForObject(product.id, true);
    try {
      final apiResponse = await productRequest.deleteProduct(product);
      //
      if (apiResponse.allGood) {
