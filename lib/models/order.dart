...
class Order {
    ...
    return Order(
      ...
      pickupTime:
          json["pickup_time"] != null
              ? Jiffy.parseFromDateTime(
                DateFormat("yyyy-MM-dd hh:mm:ss", "en").parse(
  ...
  bool get canAssignDriver {
    bool statusChecked = ["preparing"].contains(status.toLowerCase());
    if ((this.vendor?.autoAssignment ?? false) == false) {
      statusChecked =
          ![
            "failed",
            'cancelled',
    ...
    //
    final hasDelvieryInSomeWay =
        (deliveryAddressId != null || orderStops?.isNotEmpty == true);
    //
    return driverId == null &&
        statusChecked &&
        !isPaymentPending &&
        !isSerice &&
        hasDelvieryInSomeWay;
  }
